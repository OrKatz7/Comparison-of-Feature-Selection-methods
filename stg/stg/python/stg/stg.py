...
class STG(object):
    def __init__(self, device, input_dim=784, output_dim=10, hidden_dims=[400, 200], activation='relu', sigma=0.5, lam=0.1,
                ...
                task_type='classification', report_maps=False, random_state=1, extra_args=None,CancelOut = False,DotProduct=False,soft_sigmoid=False):
        ...
        self._model = self.build_model(input_dim, output_dim, hidden_dims, activation, sigma, lam, 
                                       task_type, feature_selection,CancelOut,DotProduct,soft_sigmoid)
    ...
    def build_model(self, input_dim, output_dim, hidden_dims, activation, sigma, lam, task_type, feature_selection,CancelOut,DotProduct,soft_sigmoid):
        if task_type == 'classification':
            self.metric = nn.CrossEntropyLoss()
            self.tensor_names = ('input','label')
            if feature_selection:
                return STGClassificationModel(input_dim, output_dim, hidden_dims, device=self.device, activation=activation, sigma=sigma, lam=lam,CancelOut=CancelOut,DotProduct=DotProduct,soft_sigmoid=soft_sigmoid)
            else:
